...
#define nextparam()	do { if ( ! pstr ) { geErrorLog_AddString(-1,"ProcEng_CreatePalette : params insufficient",NULL); goto fail; }; pstr = strtok(NULL,strbreakers); } while(0)
...
#define getfloat()	( pstr == NULL ) ? 0.0f : (float)atof(pstr); nextparam();
	...
	{
		geErrorLog_AddString(-1,"ProcEng_MakePalette : no palette type!",NULL);
	}
