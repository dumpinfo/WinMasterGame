...
// create a msg box for errors
#define MSG(msg) MessageBox( NULL, msg, "Application Message", MB_OK )

// Functions to build the program. All you have to do is add
// a seperate file with those functions to your project.
...
void x3d_MoveForward (D3DVECTOR v)
{
  g_pCamera->SetVelocity(g_pScene, v.x, v.y, v.z, FALSE);
}

///////////////////////////////////////////////////////////////////

void x3d_MoveBackward (D3DVECTOR v, D3DVALUE a)
{
  g_pCamera->SetVelocity(g_pScene, -v.x, -v.y, -v.z, FALSE);
}
...
void x3d_TurnRight (D3DVALUE a)
{
  g_pCamera->SetRotation (g_pScene, D3DVAL (0.0), D3DVAL (1.0), D3DVAL (0.0), a);
}

///////////////////////////////////////////////////////////////////

void x3d_TurnLeft (D3DVALUE a)
{
  g_pCamera->SetRotation (g_pScene, D3DVAL (0.0), D3DVAL (1.0), D3DVAL (0.0), -a);
}
...
void x3d_StopMove (void)
{
  g_pCamera->SetRotation (g_pScene, D3DVAL (0.0),
							  D3DVAL (0.0),
							  D3DVAL (0.0),
							  D3DVAL (0.0));
  g_pCamera->SetVelocity (g_pScene, D3DVAL(0.0), D3DVAL(0.0), D3DVAL(0.0), FALSE);
}
...
case WM_KEYDOWN:
    {
// Define the direction, up, and down vectors for motion information
      D3DVECTOR pos, direction, up, right;

// Get the orientation of the camera frame relative to the scene
      g_pCamera->GetOrientation(g_pScene, &direction, &up);

// Get the position of the camera frame relative to the scene
      g_pCamera->GetPosition (g_pScene, &pos);
            
// Calculate the vector cross product of the up and direction vectors to
// produce the right vector
      D3DRMVectorCrossProduct(&right, &up, &direction);

// Re-compute the vectors values
      up.x /= D3DVAL(4.5);
      up.y /= D3DVAL(4.5);
      up.z /= D3DVAL(4.5);
      right.x /= D3DVAL(4.5);
...
// Check for any keyboard key depression
      switch (wparam)
      {
// If the up arrow is hit, set the velocity of the camera to the value
// in the direction vector
        case VK_UP:
          x3d_MoveForward (direction);
        break;

// If the down arrow is hit, set the velocity of the camera to the reverse
// of the value in the direction vector
        case VK_DOWN:
          x3d_MoveBackward (direction, D3DVAL (0.1));
        break;
		}
    }
    break;
    ...
    case WM_KEYUP:
      switch (wparam)
      {
        case VK_UP:
        case VK_DOWN:
// Set the velocity of the camera to zero to stop it
          x3d_StopMove ();
        break;
