				 ...
				 LPDIRECT3DRMFRAME3 pScene, LPDIRECT3DRMFRAME3 pCamera )
{
	...
	AddLight(pLightsFrame, D3DRMLIGHT_AMBIENT, pLight1, 1.0f, 1.0f, 1.0f, -4.0, 20.0, 17.0);
	...
	pMeshFrame->SetPosition(lpScene, -4.0, 10.0, 20.0);

	pD3DRM->LoadTexture( "tex1.ppm", &tex);

	pD3DRM->CreateMaterial (D3DVALUE(16), &mat);

	lpMesh->SetGroupTexture(D3DRMGROUP_ALLGROUPS, (LPDIRECT3DRMTEXTURE)tex);

	lpMesh->SetGroupMaterial(D3DRMGROUP_ALLGROUPS, (LPDIRECT3DRMMATERIAL)mat);

	pD3DRM->CreateWrap(D3DRMWRAP_FLAT,
						NULL,
						...
						D3DVALUE(1),D3DVALUE(1),
						&wrap);

	wrap->Apply((LPDIRECT3DRMOBJECT) lpMesh);
    ...
    pCamera->SetPosition( pScene, -4.0, 11.0, 14.0 );
	...
	lpScene->SetSceneBackground(D3DRMCreateColorRGB(D3DVALUE(255), D3DVALUE(255), D3DVALUE(255)));

	lpScene->SetSceneFogMode(D3DRMFOG_LINEAR);
	lpScene->SetSceneFogParams(D3DVALUE(10), D3DVALUE(100), D3DVALUE(0));
	lpScene->SetSceneFogColor(D3DRMCreateColorRGB(D3DVALUE(255), D3DVALUE(255), D3DVALUE(255)));
	lpScene->SetSceneFogEnable(TRUE);
